...
public class GUI {
   ...
   *
   * @param preds Предсказания
   */
  public void updateResults(double[] preds) {
    // Делаем СтрингБилдер
    StringBuilder sb = new StringBuilder();
    sb.append("<html>");
    sb.append("<p>Результаты :</p>");
    for (int i = 0; i < preds.length; i++) {
      sb.append("<p> Модель № ")
          .append(i)
          .append(" предсказала результат : ")
          .append(df.format(preds[i]))
          .append(" кг</p>");
    }
    sb.append("</html>");

    // Устанавливаем новый текст
    lblResults.setText(sb.toString());
  }
}
